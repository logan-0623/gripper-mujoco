...
主研究对象为同一训练谱系中能力不同的 SmolVLA checkpoints；各 checkpoint 在分析期间冻结，不等于训练期间 VLM 冻结。先限定 LIBERO 任务、checkpoint、测量面板和资源预算，在尚无可靠发现时不强制指定某一种交互能力；有线索后再收紧候选和行为假设。不以方法排名、成功率超过官方、动作扰动最大或 gate passed 作为科学目标。所谓“物理信息”必须通过几何、接触关系、动力学相关条件或可操作的行为测量获得依据，不能仅由训练位移或动作 RMS 命名。
...
无标签差分与监督读出均可作为候选来源，不要求无标签发现先成功。Contact、StableGrasp 等可用于外部测量和候选解释，不规定模型必须具有同名内部概念；使用监督标签定位方向时须记录选择数据与独立验证，不能用同一批样本同时挑选并确认。当前主问题是能力获得中的计算变化与功能作用，不要求未来预测通过；只有声明 predictive interaction state 时，才必须恢复独立未来预测与信息预算比较。
...
### 0.3 第一、二阶段：并行观察行为变化与发现候选计算

并行检查已有逐任务、逐 episode 的行为轨迹与内部表示，不要求先确定唯一目标行为，不用总体成功率替代具体行为。保留失败、退步与任务异质性，不只选择后期成功的轨迹。离线扫描优先复用 5k/10k/15k/20k/25k 五个 checkpoint；资源受限的 smoke 可先用两个点，但不据此宣布能力涌现。深查阶段再据行为证据选择早期、过渡期和成熟期。5k 不等于预训练初始化，官方权重是外部参照，不与自训练权重拼成同一训练序列。
...
| 顺序 | 本轮交付 | 边界 |
| --- | --- | --- |
| 1 | 并行交付逐任务行为表、checkpoint CKA 图谱与读出迁移矩阵，核验谱系和样本身份 | 不要求事先确定唯一行为，不自动把 5k→10k 命名为抓取涌现 |
...
**本轮优化的核心：限定探索预算而不预定答案，围绕能力变化区分信息可访问性增长、重新组织与功能性招募。** 历史结果保持原样；实现范围见下节，不把计划当作已完成实测或启动授权。

### 0.8 五方法探索路线与本地实现范围（2026-09-22）

执行顺序为 **①② 并行发现 → ③⑤ 判别解释 → ④ 定向闭环**。这不是必须连续通过正结果的门禁。CKA 稳定而功能变化、线性读出迁移失败、干预无效均可保留为线索；不以更换指标或加大剂量追逐预期结果。

| 方法 | 当前执行与需要回答的问题 | 实现边界 |
| --- | --- | --- |
...
```bash
# 路径为示意；替换为实际已完成的缓存目录，按训练步顺序列出。
bash scripts/python.sh -W error::RuntimeWarning \
  -m interaction_vla.representation_study.libero.checkpoint_readouts \
  --bank /path/to/state_bank \
  --checkpoint 5k /path/to/5k_train_trace /path/to/5k_validation_trace \
  --checkpoint 10k /path/to/10k_train_trace /path/to/10k_validation_trace \
  --checkpoint 15k /path/to/15k_train_trace /path/to/15k_validation_trace \
  --checkpoint 20k /path/to/20k_train_trace /path/to/20k_validation_trace \
  --checkpoint 25k /path/to/25k_train_trace /path/to/25k_validation_trace \
  --output outputs/acquisition/checkpoint_readouts_exploration
```

产物为不可覆盖的 `report.json`：包含缓存 binding hash、比较合同、CKA、逐任务及 task-macro 的迁移 MSE、训练均值基线、时间/robot 单独读出。固定 Ridge α=10，不用 validation 选择超参数；源 checkpoint 的训练集拟合 scaler，迁移时不重新拟合。二值标签也报告未截断 MSE，不称校准概率或分类准确率。权重为 task 等权、task 内 episode 等权、episode 内 state 等权；噪声重复先取均值，不当独立样本。
