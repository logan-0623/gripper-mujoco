...
- **Replay drift.** Annotation uses the preregistered `teacher_forced_one_step_qpos` protocol: every label frame is restored from its recorded state, and its action must reproduce the next recorded configuration within the registered qpos tolerances. This validates configuration-level alignment but does not claim velocity-level or long-horizon open-loop dynamic equivalence.
