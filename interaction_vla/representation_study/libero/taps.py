    ...
    def capture(
        ...
        metadata = {
            "vision_output": {
                ...
                "raw_shapes": [list(value.shape) for value in image_outputs],
                "pooled_shape": list(result["vision_output"].shape),
            },
            "multimodal_fusion": {
                "module": "model.vlm_with_expert.forward",
                "pooling": "valid_token_mean(prefix_pad_masks)",
                "call_selection": "first_prefix_prefill",
                "raw_shape": list(prefix_hidden.shape),
                "mask_shape": list(prefix_mask.shape),
                "pooled_shape": list(result["multimodal_fusion"].shape),
            },
            "action_expert_input": {
                "module": "model.action_time_mlp_out",
                "pooling": "action_chunk_mean",
                "call_selection": "final_denoising",
                "raw_shape": list(action_inputs[-1].shape),
                "pooled_shape": list(result["action_expert_input"].shape),
            },
            "pre_action": {
                "module": "model.action_out_proj:input",
                "pooling": "action_chunk_mean",
                "call_selection": "final_denoising",
                "raw_shape": list(pre_actions[-1].shape),
                "pooled_shape": list(result["pre_action"].shape),
            },
        }
